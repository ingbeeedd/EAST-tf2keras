...
def generator(FLAGS, input_size=512, background_ratio=3./8, is_train=True, idx=None, random_scale=np.array([0.5, 1, 2.0, 3.0]), vis=False):
  if is_train:
    ...
    image_list = np.array(get_images(FLAGS.valid_data_path))
    if not idx is None:
      image_list = image_list[idx]
    print('{} valid images in {}'.format(
      image_list.shape[0], FLAGS.valid_data_path))
...
def count_samples(data_path):
  if sys.version_info >= (3, 0):
    return len([f for f in next(os.walk(data_path))[2] if f[-4:] == ".jpg"])
  else:
