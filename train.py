...
parser.add_argument('--nb_workers', type=int, default=8) # number of processes to spin up when using process based threading, as defined in https://keras.io/models/model/#fit_generator
...
class CustomTensorBoard(TensorBoard):
    ...
    '''
    def on_epoch_end(self, epoch, logs=None):        
        logs.update({'learning_rate': tf.keras.backend.eval(self.model.optimizer.lr), 'small_text_weight': tf.keras.backend.eval(self.small_text_weight)})
...
class SmallTextWeight(Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        tf.keras.backend.set_value(self.weight, 0)
...
def main(argv=None):
    ...
    val_data_generator = data_processor.val_generator(FLAGS)
    ...
    score_map_loss_weight = tf.keras.backend.variable(0.01, name='score_map_loss_weight')
    small_text_weight = tf.keras.backend.variable(0., name='small_text_weight')
    ...
    history = parallel_model.fit(train_data_generator,
                                epochs=FLAGS.max_epochs, 
                                steps_per_epoch=train_samples_count/FLAGS.batch_size,
                                validation_data=val_data_generator,
                                validation_steps=val_samples_count/FLAGS.batch_size,
